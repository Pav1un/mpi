...
void main(int argc, char ** argv)
{
	 ...
	 /* функция определения номера процесса. Номер процесса записывается в переменную myrank. */
	 MPI_Comm_size (MPI_COMM_WORLD, &ranksize); 
	 /* Определение общего числа параллельных процессов в группе ranksize - размер группы .*/

	 if (myrank == 0)               /* Если 0 (мастер) процесс */
	 {
	 ...
	 /* Синхронизация процессов.Блокирует работу процессов, вызвавших данную процедуру, до тех пор,*/
	 /*пока все оставшиеся процессы группы comm также не вызовут эту функцию.*/ 
...
OUT status - параметры принятого сообщения*/ 
	 ...
	 /* Вычислить свою часть интервала */
	 t4 = MPI_Wtime();
